// Validate the statute-section required-source guard.
// Rerun Q01, Q02, Q03, Q18, and BGZ 5555/18.
...
const OUT = "reports/quality-audit/runs-statute-section-guard";
...
console.log(`[guard] triggering ${set.length} runs`);
...
const results = await Promise.all(triggered.map(async ({ q, run_id }) => {
  ...
  const ra: any = md?.required_anchors ?? {};
  const raw = {
    ...base,
    ok: d.ok === true,
    ...
    required_anchors: (ra.statuses ?? []).map((s: any) => ({
      anchor_id: s.anchor_id,
      description: s.description,
      is_statute_section_anchor: s.is_statute_section_anchor ?? false,
      is_docket_anchor: s.is_docket_anchor ?? false,
      status: s.status,
      verified_support: s.verified_support,
    })),
    answer: row.answer ?? "",
  ...
  console.log(
    `[${q.id}] ok=${raw.ok} shape=${raw.answer_intent?.output_shape ?? "-"} lead=${raw.lead_ref?.ref ?? "-"}/${raw.lead_ref?.reason ?? "-"} anchors=${raw.required_anchors.map((a: any) => `${a.anchor_id}:${a.status}`).join(",")}`,
  );
  return raw;
}));

...
const IDS = (process.env.E2_IDS ?? "L2,L3,L4,L5,L6").split(",");
...
const results = await Promise.all(
  FIXTURES.map(async (fx) => {
    ...
    await Bun.write(`reports/legal-research-v1-p7-phaseE2-${fx.id}.json`, JSON.stringify(summary, null, 2));
...
for (const fx of ALL) {
  try {
    const f = Bun.file(`reports/legal-research-v1-p7-phaseE2-${fx.id}.json`);
    if (await f.exists()) all.push(await f.json());
  } catch {}
}
await Bun.write(`reports/legal-research-v1-p7-phaseE2-summary.json`, JSON.stringify(all, null, 2));

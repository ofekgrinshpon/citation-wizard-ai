...
while (Date.now() < deadline) {
  ...
  if (q.ok) { const rows = await q.json(); if (Array.isArray(rows) && rows.length && (rows[0].metadata?.trace_status === "terminal" || (rows[0].answer && rows[0].answer.length > 200))) { row = rows[0]; break; } }
  await new Promise((x) => setTimeout(x, 5000));
}
mkdirSync("reports/integrity-downgraded-judgment-body-probe-v1", { recursive: true });
writeFileSync("reports/integrity-downgraded-judgment-body-probe-v1/run.json", JSON.stringify({ run_id: t.run_id, row }, null, 2));

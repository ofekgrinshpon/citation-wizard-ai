...
const OUT = "reports/facet-validation";
...
for (const q of QUERIES) {
  ...
  const rec = {
    ...
    claim_facet_expansion: md.claim_facet_expansion ?? d.claim_facet_expansion ?? null,
  ...
  results.push(rec);
  const cfe: any = rec.claim_facet_expansion;
  console.log(
    `[${q.id}] facets=${cfe?.enabled ? cfe.facet_count : "off"} no_primary=${cfe?.facets_without_primary_support ?? "-"} pass=${rec.pass} status=${rec.terminal_status} branch=${branch} drafted=${rec.drafted} len=${answer.length} used=${used} judg=${judgments.length} body=${judgBody.length} metaonly=${judgMetaOnly.length} statute_acq=${statuteAcquiredCount} ms=${rec.ms} ${failures.join(",")}`,
  );
  writeFileSync(`${OUT}/${q.id}.json`, JSON.stringify(rec, null, 2));
  writeFileSync(
    `${OUT}/summary.json`,

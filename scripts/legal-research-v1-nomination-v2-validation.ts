// nomination_toolcall_reliability_v1 — sequential 7-run validation.
...
const OUT = "reports/nomination-identifier-bearing-rate";
...
for (const q of QUERIES) {
  ...
  console.log(`[${q.id}] terminal=${rec.terminal} nom_cands=${rec.source_nomination.candidates_count} act=${rec.source_nomination.actionable_count} exp=${rec.source_nomination.exploratory_count} esc=${rec.source_nomination.escalated} parse_error=${rec.source_nomination.parse_error} fn=${rec.footnotes_count} ${rec.ms}ms`);
}

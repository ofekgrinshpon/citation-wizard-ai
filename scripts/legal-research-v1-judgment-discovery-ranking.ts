...
const ALL = [
  ...
  { id: "R10", query: `מה הפסיקה אומרת על הרמת מסך בחברות משפחתיות?` },
...
const OUT = "reports/judgment-discovery-ranking";
...
function summarize(q: { id: string; query: string }, run_id: string, row: any, ms: number) {
  ...
  return {
    ...
    sufficiency: suf,
    acquisition: acq,
    judgment_discovery: ret?.judgment_discovery ?? null,
...
for (let i = 0; i < QUERIES.length; i += CONCURRENCY) {
  ...
  const out = await Promise.all(batch.map(async (q) => {
    ...
    const jd: any = rec.judgment_discovery ?? {};
    console.log(`[${q.id}] mode=${rec.research_mode} branch=${rec.deterministic_branch} suff=${rec.sufficiency?.reason} usable_j=${rec.usable_judgments.length} disc_q=${(jd.judgment_discovery_queries ?? []).length} off_found=${jd.official_judgment_documents_found} off_acq=${jd.official_judgment_documents_acquired} inst_excl=${jd.institutional_pages_excluded_before_budget} skipped=${jd.skipped_higher_quality_candidates} comm=${jd.commentary_share_of_admitted_pack} len=${rec.answer_len}`);
    return rec;
  }));
  results.push(...out);

// judgment_nomination_coverage_for_named_dockets_v1 — sequential 9-run validation.
...
const OUT = "reports/docket-coverage";
...
for (const q of QUERIES) {
  ...
  const edg: any = d.explicit_docket_guard ?? md.explicit_docket_guard ?? {};
  ...
  console.log(`[${q.id}] terminal=${rec.terminal} egress_calls=${rec.court_egress.calls} guard=${edg.added_count ?? "-"}/${edg.merged_count ?? "-"}/${edg.detected_count ?? "-"} suppressed=${ju.suppressed ?? "-"} saved=${ju.relay_slots_saved ?? "-"} bodies=${rec.official_discovery.bodies_acquired} fn=${rec.footnotes_count} dangling=${rec.dangling_markers} ${rec.ms}ms`);
}

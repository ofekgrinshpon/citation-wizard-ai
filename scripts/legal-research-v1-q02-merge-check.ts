// Final merge-check validation: Q18, Q03, Q01, BGZ5555, Q02
...
const OUT = "reports/quality-audit/runs-q02-merge-check";
...
console.log(`[merge-check] triggering ${set.length} runs`);
...
const results = await Promise.all(triggered.map(async ({ q, run_id }) => {
  ...
  const raw = {
    ...
    required_anchors: md?.required_anchors ?? null,
    answer: row.answer ?? "",
  ...
  console.log(
    `[${q.id}] ok=${raw.ok} shape=${raw.answer_intent?.output_shape ?? "-"} lead_ref=${raw.lead_ref?.ref ?? "-"} reason=${raw.lead_ref?.reason ?? "-"} caveat=${raw.missing_anchor_caveat_injected}`,
  );
  return raw;
}));

writeFileSync(`${OUT}/_summary.json`, JSON.stringify(results, null, 2));
console.log(`[merge-check] done`);

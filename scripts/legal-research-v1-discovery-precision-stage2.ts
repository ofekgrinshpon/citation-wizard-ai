...
const OUT = "reports/discovery-precision";
...
for (const run of RUNS) {
  ...
  results.push(rec);
  writeFileSync(`${OUT}/dp-stage2-${run.id}.json`, JSON.stringify(rec, null, 2));
  console.log(
...
writeFileSync(
  `${OUT}/dp-stage2-SUMMARY-${process.env.STAGE2_PHASE ?? "1"}.json`,
  JSON.stringify(
    results.map((r) => ({
      id: r.id,
      pass: r.pass,

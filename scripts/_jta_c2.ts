...
const OUT = "reports/judgment-text-acquisition";
...
const rows = await Promise.all(triggered.map(async ({ id, query, run_id }) => {
  ...
  const summary = {
    ...
    judgment_text_acquisition: md?.retrieval?.judgment_text_acquisition ?? null,
...
console.log("[judgment-acquisition] done ->", OUT);

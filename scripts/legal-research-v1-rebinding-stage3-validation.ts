...
const ALL = [
  { id: "ACADEMIC", query: `אני כותב סמינריון על עילת הסבירות במשפט המנהלי — אילו מקורות מרכזיים כדאי לקרוא?` },
  { id: "NATION-STATE-ACADEMIC", query: `כיצד מנתחת הספרות האקדמית את חוק יסוד: ישראל – מדינת הלאום של העם היהודי ואת שאלת הזהות החוקתית?` },
  { id: "PAYWALL", query: `מה נכתב במאמרים בכתב העת "עיוני משפט" על תורת הפרשנות התכליתית של אהרן ברק?` },
  { id: "MMM", query: `מה עולה ממחקרי מרכז המחקר והמידע של הכנסת ומדוחות רגולטוריים בנוגע לאכיפת חוקי עבודה בישראל?` },
  { id: "B8", query: `מהי דוקטרינת ההבטחה המנהלית?` },
  { id: "D1", query: `מהם מבחני המידתיות בביקורת חוקתית בישראל, וכיצד יישם אותם בית המשפט העליון?` },
  { id: "D3", query: `מה הדין לגבי פיטורי עובדת בהיריון, ומהו היקף ההגנה?` },
  { id: "DARKPATTERNS", query: `מהי הגישה המשפטית והאקדמית ל"דפוסים אפלים" (dark patterns) בהגנת הצרכן, ואילו מקורות מחקריים דנים בכך?` },
  { id: "R02", query: `מה נקבע בע"א 6821/93 בנק המזרחי המאוחד נ' מגדל כפר שיתופי?` },
  { id: "P02", query: `מה נקבע בבג"ץ 9999/99 פלוני נ' אלמוני?` },
];
...
for (const q of QUERIES) {
  ...
  results.push(rec);
  writeFileSync(`${OUT}/stage3-${q.id}.json`, JSON.stringify(rec, null, 2));
  console.log(
...
writeFileSync(`${OUT}/stage3-SUMMARY.json`, JSON.stringify(

...
const OUT = "reports/large-scholarship-pdf-extraction-v1";
...
for (const q of QUERIES) {
  ...
  results.push({
    ...
    large_pdf_version: d.large_scholarship_pdf_version ?? null,
    large_pdf_eligibility: d.large_scholarship_pdf_eligibility ?? [],
    large_pdf_attempt: d.large_scholarship_pdf_extraction_attempt ?? [],
    large_pdf_identity: d.large_scholarship_pdf_identity_check ?? [],
    large_pdf_quality: d.large_scholarship_pdf_body_quality ?? [],
    large_pdf_cache: d.large_scholarship_pdf_cache_write ?? [],
    large_pdf_latency_ms: d.large_scholarship_pdf_added_latency_ms ?? 0,
    large_pdf_stop_reason: d.large_scholarship_pdf_stop_reason ?? null,
    large_pdf_downstream: d.large_scholarship_pdf_downstream_effect ?? [],
  ...
  console.log(
    `[${q.id}] large_pdf stop=${d.large_scholarship_pdf_stop_reason} attempts=${(d.large_scholarship_pdf_extraction_attempt??[]).length} improved=${(d.large_scholarship_pdf_downstream_effect??[]).filter((x:any)=>x.extraction_succeeded).length}`,
  );
  console.log(

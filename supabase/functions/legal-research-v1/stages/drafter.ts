...
- העדף מינוח משפטי ישראלי מדויק (למשל "השתק פלוגתא", "צו מניעה זמני", "פיצוי מוסכם", "סבירות", "הבטחה מנהלית", "השתק עילה"). אל תשתמש במונחים באנגלית כאשר קיים מונח עברי טבעי.
- מבנה רצוי: פתח בהגדרת הדוקטרינה / הכלל, המשך בתנאיו / יסודותיו, ואז ביישומו ובסייגיו ובחריגיו. כשיש מחלוקת בפסיקה או בספרות — ציין זאת.
- התשובה צריכה להיות **מפותחת ומהותית**, לא תקציר קצר. אין מגבלת מילים נוקשה, אך אל תהיה תמציתי מדי: אורך התשובה ייקבע לפי כמות ועומק המקורות המאומתים שסופקו, וצריך לאפשר מענה משפטי משמעותי לשאלה. אל תקצר באופן מלאכותי ואל תמתח באופן מלאכותי.
...
function buildUserMessage(
  ...
  lines.push("");
  lines.push(
    "כתוב תשובה משפטית-אקדמית מפותחת בעברית, מבוססת אך ורק על המקורות שסופקו, עם הערות שוליים בכתב עילי. מספר אך ורק מקורות שאתה באמת מצטט. אל תקצר באופן מלאכותי — נצל את עומק המקורות המאומתים כדי לתת מענה משפטי משמעותי לשאלה.",
  );
  return lines.join("\n");
}

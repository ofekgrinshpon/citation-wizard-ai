...
const SYSTEM_PROMPT = `אתה כותב משפטי ישראלי. תקבל שאלת משתמש, רשימת טענות (claims), ורשימת מקורות מאומתים בלבד.
כל מקור מסומן ב-ref כגון s1, s2, s3 ועוד.
המשימה: לכתוב תשובה משפטית קצרה ומועילה בעברית, עם הערות שוליים מספריות.
...
- אם תמיכה במקור היא partial בלבד או דקה — נסח בזהירות ("יש הסוברים", "ככלל", "במקרים מסוימים"), או השמט את הטענה.
...
- אורך מומלץ: 120–300 מילים.

פורמט החזרה: רק דרך הקריאה לכלי emit_draft.
- answer_markdown: טקסט התשובה בעברית, עם מספרי הערות בכתב עילי (¹ ² ³ …).
- used_sources: רשימת המקורות שבהם השתמשת בפועל. כל פריט כולל ref (כפי שסופק לך), number (המספר שמוופיע בתשובה), ו-candidate_id (כפי שסופק לך). המספרים יהיו רציפים מ-1 ולפי סדר ההופעה הראשונה בתשובה.`;
...
function buildUserMessage(
  ...
  lines.push("");
  lines.push(
    "כתוב תשובה משפטית בעברית עם הערות שוליים בכתב עילי. מספר אך ורק מקורות שאתה באמת מצטט.",
  );
  return lines.join("\n");
}

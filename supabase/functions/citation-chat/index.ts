...
const CITATION_ENGINE_TEMPLATES: Record<string, { rule: string; template: string; required: string[] }> = {
  ...
  "הצעת חוק": {
    rule: "כלל 8",
    template: 'הצעת חוק {שם}, {שנה עברית}-{שנה לועזית}, ה"ח {הכנסת/הממשלה/ריק} {מספר חוברת}, {עמוד}.',
    required: ["שם הצעת החוק", "שנה עברית", "שנה לועזית", "מספר חוברת", "עמוד ראשון"],
    notes: 'אחרי ה"ח אפשר לכתוב "הכנסת" או "הממשלה" או להשמיט (ריק). סוג החוברת אינו חובה.',
  },
  "הצעת חוק יסוד": {
    rule: "כלל 8",
    template: 'הצעת חוק-יסוד: {שם}, {שנה עברית}, ה"ח {הכנסת/הממשלה/ריק} {מספר חוברת}, {עמוד}.',
    required: ["שם הצעת חוק היסוד", "שנה עברית (ללא שנה לועזית)", "מספר חוברת", "עמוד ראשון"],
    notes: 'בהצעות חוק יסוד מציינים שנה עברית בלבד. אחרי ה"ח אפשר "הכנסת" או "הממשלה" או להשמיט.',
  },
  "פסיקה (דפוס)": {
    rule: "כלל 18",
...
- הצעות חוק רגילות (כלל 8): שם הצעת החוק, שנה עברית ולועזית, מספר חוברת, עמוד ראשון. הקיצור התקני הוא ה"ח (לא הצ"ח!). אחרי ה"ח כותבים "הכנסת" או "הממשלה" או משמיטים (ריק) — תלוי בסוג החוברת.
- הצעות חוק יסוד (כלל 8): שם הצעת חוק היסוד, שנה עברית בלבד (ללא שנה לועזית!), מספר חוברת, עמוד ראשון. הקיצור התקני הוא ה"ח. אחרי ה"ח כותבים "הכנסת" או "הממשלה" או משמיטים.

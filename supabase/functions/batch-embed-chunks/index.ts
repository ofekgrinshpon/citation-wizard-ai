...
async function embedSubBatch(texts: string[], apiKey: string): Promise<(number[] | null)[]> {
...
serve(async (req) => {
  ...
  try {
    ...
    for (let i = 0; i < chunks.length; i += SUB_BATCH_SIZE) {
      ...
      const embeddings = await embedSubBatch(texts, OPENAI_API_KEY);
      ...
      for (let j = 0; j < subBatch.length; j++) {
        if (embeddings[j]) {
          ...
          if (updateErr) {
            console.error(`Update error for chunk ${subBatch[j].id}:`, updateErr);
            failed++;
          } else {
            processed++;
          }

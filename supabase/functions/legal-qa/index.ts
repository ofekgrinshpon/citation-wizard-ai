...
${contractOn ? `
═══ חוזה ציטוט Card→Claim — חובה מוחלטת ═══
1. לאחר **כל** קביעה משפטית מהותית הוסף סמן בצורה: \`[cite:S#]\` (אפשר לאחד מקורות: \`[cite:S1,S3]\`).
2. **אל תכתוב בלוק "--- הערות שוליים ---" ידני.** המערכת תייצר את כל הערות-השוליים אוטומטית מתוך סמני \`[cite:S#]\` שתשתמש בהם.
3. **אל תשתמש כלל בסמני \`[N]\` הישנים בסוגריים מרובעים.** רק \`[cite:S#]\`.
4. השתמש אך ורק במזהי \`{S#}\` המופיעים ברשימת המקורות למטה. **אל תמציא** מזהים שלא קיימים.
5. אם אין במקורות תמיכה לטענה — או שלא תכתוב את הטענה כלל, או שתנסח אותה כאי-ודאות מפורשת ("ייתכן", "טרם הוכרע") **בלי** סמן.

דוגמאות:
✓ נכון: "הלכת אפרופים הרחיבה את גישת הפרשנות התכליתית של חוזים [cite:S3]."
✓ נכון: "החוק מסדיר זאת במפורש [cite:S1,S2]."
✗ שגוי: "הלכת אפרופים הרחיבה את גישת הפרשנות התכליתית של חוזים¹"
✗ שגוי: "הלכת אפרופים הרחיבה את גישת הפרשנות התכליתית של חוזים [1]."
✗ שגוי: לכתוב בסוף התשובה בלוק "--- הערות שוליים ---" עם רשימה ממוספרת ידנית.

לסיכום: בגוף התשובה משובצים סמני \`[cite:S#]\` בלבד; אין בלוק הערות שוליים בכלל.
` : `
═══ חובת פלט מוחלטת ═══
...
${contractOn
  ? `- **יעד סמני [cite:S#] מעוגנים: ${fnFloor}-${fnMax}**, מינימום ${fnFloor}. "מעוגן" = יש כרטיס מקור ברשימה למטה ש-{S#} שלו תומך בטענה. אם אין מקור תומך — אל תוסיף סמן ואל תכניס טענה הדורשת אסמכתא. **אל תמציא** מזהי {S#}.`
  : footnoteFloorBlock}
...
לפני שאתה מסיים, ודא:
${contractOn
  ? `1. כל סמן \`[cite:S#]\` בגוף מתייחס למזהה \`{S#}\` ברשימת המקורות למטה. אם אין כרטיס מתאים — מחק את הסמן ושכתב את המשפט כדעה כללית בלי סמן.
2. הגוף ${researchDepth === "deep" ? `מעל ${wordMin} מילים` : `בטווח ${wordMin}-${wordMax} מילים`}, ויש לפחות ${fnFloor} סמני \`[cite:S#]\` שונים מעוגנים.
3. **אין בתשובה בלוק "--- הערות שוליים ---" ידני, ואין סמני \`[N]\` בסוגריים מרובעים.**`
...
4. אין כותרות "שורה תחתונה" / "השלכות מעשיות" / "Bottom line". המבנה הוא: פתיחה (ללא כותרת) → מסגרת נורמטיבית → ${deepHasSubIssues ? "סעיף לכל תת-סוגיה" : "ניתוח"} → סיכום הפרק.
5. כל פסקה היא 4–7 משפטים של פרוזה. אין פסקת משפט-אחד. אין bullets.` : researchDepth === "deep" ? `
4. כל חמשת החלקים נוכחים: שורה תחתונה, מסגרת נורמטיבית (≥3 פסקאות), ${deepHasSubIssues ? "סעיף לכל תת-סוגיה (≥3 פסקאות, ≥250 מילים כל אחד)" : "ניתוח מעמיק (≥3 פסקאות, ≥250 מילים)"}, השלכות מעשיות (≥2 פסקאות), מסקנה.` : ""}
...
- needsPinpoint=true: חובה pinpoint ב${contractOn ? "הקשר הציטוט (יופיע אוטומטית בהערה הנגזרת)" : "הערת השוליים (ס' X ל..., בעמ' Y)"}.
...
- כל פרט ביבליוגרפי מופיע **רק** ב${contractOn ? "הערה הנגזרת אוטומטית מסמן [cite:S#]" : "הערת השוליים"}.
- כשמזכירים מקור בגוף בשם נרטיבי, ${contractOn ? "סמן [cite:S#] שאחריו חייב להפנות לאותו מקור בדיוק." : "הערת השוליים המתאימה חייבת להכיל את אותו מקור בדיוק."}

${contractOn ? `═══ סימני הפניה ═══
- רק \`[cite:S#]\` (או \`[cite:S1,S3]\`). אחרי הפיסוק, לא לפניו. דוגמה: "...ההלכה הקיימת. [cite:S3]".
- **אסור** \`[N]\`, \`(1)\`, מספרי-על ¹²³ ידניים, או בלוק הערות שוליים.
` : `═══ סימני הפניה ═══
...
`}
═══ כללי ברזל ═══
- אסור ${contractOn ? "סמן [cite:S#]" : "הערה"} ללא anchor אמיתי. אין מקור — אין סמן.

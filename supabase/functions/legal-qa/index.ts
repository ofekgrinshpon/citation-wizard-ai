...
${JSON.stringify(claimMap.filter((c) => c.allowed_to_state).map((c) => ({
  ...
  needs_pinpoint: c.needs_pinpoint,
    ...
    const buildCompactStructuredPrompt = (): string => {
      ...
      const structureBlock = deepHasSubIssues
        ? `═══ מבנה התשובה למצב Deep (חובה — קרא ויישם) ═══
מבנה דינמי לפי תת-הסוגיות מהמפה. כתוב לפי הסדר הזה, עם הכותרות המדויקות הללו ב-**bold**:
...
פסקה מהותית (${frameworkSentenceTarget}). זהה את החוקים, פסקי הדין וההלכות הרוחביים שמסדירים את הסוגיה כולה לפני הצלילה לתת-הסוגיות. אזכר אותם בשם נרטיבי ("חוק X", "בעניין Y") עם [N] לאחר כל מקור מהותי.
...
- **לפחות 3 פסקאות מהותיות** בכל סעיף (לא משפט-שניים).
...
2. הגוף בטווח ${wordMin}-${wordMax} מילים, ויש לפחות ${fnFloor} הערות מעוגנות.${deepHasSubIssues ? `
3. כל סעיף תת-סוגיה כולל לפחות 3 פסקאות מהותיות.` : ""}

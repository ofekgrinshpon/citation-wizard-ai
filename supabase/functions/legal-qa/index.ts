  ...
  needs_pinpoint: c.needs_pinpoint,
    ...
    const buildCompactStructuredPrompt = (): string => {
      ...
      const uncoveredLine = claimMapV2 && claimMapV2.uncoveredSubIssues.length > 0
        ...
        : "";
      // ─── Pilot v7.6 (Fast-mode shaping) ───────────────────────────
      // Genuinely shorter Fast output: bottom-line first, 400–700 words,
      // 4–6 anchored citations. The previous 800–1500 prompt was producing
      // the same mini-memo as the legacy path, just with structured routing.
      // Fast must FEEL different in shape, density, and time-to-answer —
      // not just be a faster version of the same artifact.
...
**מסגרת נורמטיבית**
פסקה אחת קצרה (4-7 משפטים). זהה את החוקים, פסקי הדין וההלכות המרכזיים שמסדירים את הסוגיה. אזכר אותם בשם נרטיבי ("חוק X", "בעניין Y") עם [N] לאחר כל מקור מהותי.

**יישום**
פסקה אחת קצרה (4-7 משפטים). יישם את המסגרת על השאלה הקונקרטית שהוצגה. ניתוח ממוקד, לא חזרה על המסגרת. הוסף [N] לכל טענה שמסתמכת על מקור.

**מסקנה**
1-2 משפטים. סגור את הטיעון. אם הוצגה אי-ודאות בשורה התחתונה — חזור עליה כאן בקצרה.

מגבלות אורך **מחייבות**:
- סך הכל **400-700 מילים** בגוף (לא כולל הערות שוליים). תשובה ארוכה יותר נחשבת כשלון.
- **4-6 הערות שוליים מעוגנות**. לא פחות (תשובה תת-מתועדת), לא יותר (Fast לא צריך 10 הערות).
...
═══ תזכורת אחרונה ═══
זהו מצב **Fast**: 400-700 מילים, 4-6 הערות מעוגנות, ארבע כותרות בלבד, בלוק הערות שוליים בסוף. אל תחרוג. אל תוסיף סעיפים. אל תכפיל את הניתוח.

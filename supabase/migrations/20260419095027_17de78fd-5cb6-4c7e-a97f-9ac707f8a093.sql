...
BEGIN
  ...
  chunk_cands AS (
    SELECT c2.document_id AS doc_id
  ...
  cand_docs AS (
    SELECT doc_id FROM chunk_cands
    UNION
  ...
  SELECT
    ...
    (
      ...
      + coalesce(c.content_rank, 0)
      + CASE
          WHEN to_tsvector('simple',
                 d.title || ' ' || d.citation || ' ' ||
                 coalesce(d.case_number,'') || ' ' || coalesce(d.court,'')
               ) @@ q.tsq
            AND coalesce(c.content_rank, 0) > 0
          THEN 0.1
          ELSE 0
        END
    )::float AS similarity
  FROM cand_docs cd
  JOIN public.legal_documents d ON d.id = cd.doc_id
  CROSS JOIN q
  LEFT JOIN LATERAL (

-- E.4b: Rewrite search_legal_chunks_text to remove per-document LATERAL loop.
-- Old shape: select ~640 candidate docs, then LATERAL per doc rescanning all
-- chunks of that doc and recomputing to_tsvector('simple', content). Cost
-- 4–7s sequentially, frequent 20s timeouts under concurrency.
--
-- New shape: rank candidate chunks via the existing FTS GIN index, pick the
-- top chunk per document with ROW_NUMBER(), then join legal_documents and
-- combine with the meta-only match path. No per-doc inner loop.
...
BEGIN
  ...
  -- Pre-rank candidate chunks once via the FTS GIN index.
  chunk_cands AS (
    SELECT
      c2.id          AS chunk_id,
  ...
  -- Top chunk per document, computed on the limited candidate set only.
  top_chunk_per_doc AS (
    SELECT chunk_id, doc_id, content_rank
    FROM (
      SELECT chunk_id, doc_id, content_rank,
             ROW_NUMBER() OVER (PARTITION BY doc_id ORDER BY content_rank DESC) AS rn
      FROM chunk_cands
    ) s
    WHERE rn = 1
  ),
  -- Documents whose metadata matches (GIN-indexed concat).
  meta_cands AS (
    SELECT
      d.id AS doc_id,
      ts_rank(
  ...
  all_docs AS (
    SELECT doc_id FROM top_chunk_per_doc
    UNION
    SELECT doc_id FROM meta_cands
  )
  SELECT
    c.id           AS chunk_id,
    d.id           AS document_id,
  ...
  FROM all_docs ad
  JOIN public.legal_documents d        ON d.id = ad.doc_id
  LEFT JOIN top_chunk_per_doc tc       ON tc.doc_id = ad.doc_id
  LEFT JOIN meta_cands m               ON m.doc_id = ad.doc_id
  LEFT JOIN public.legal_document_chunks c ON c.id = tc.chunk_id

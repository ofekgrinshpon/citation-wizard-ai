...
-- Update the identity key function to include publication source + initial page for laws
...
BEGIN
  ...
  IF coalesce(_source_type, '') IN ('legislation_primary', 'legislation_secondary') THEN
    ...
    -- Extract publication source (ס"ח or ק"ת) and the page number that follows
    ...
    IF extracted_law_name <> '' THEN
      -- Build key: law:[name]|[year]|[pub]|[page]
...
-- Recompute all existing identity keys
...
-- Deduplicate again after recomputation
WITH ranked AS (
  SELECT
    id,
    identity_key,

...
DECLARE
  ...
  ts_top1 tsquery;
  ts_precise tsquery;
  precise_cnt integer := 0;
  has_hit boolean := false;
  -- Ranking the full match set is only affordable while the set is bounded.
  precise_rank_budget constant integer := 60000;
BEGIN
  SELECT array_agg(w)
  INTO words
  ...
  -- Precise candidate lane: the conjunctive form of the same query. Its match
  -- set is small enough to rank in full, so relevance ordering happens BEFORE
  -- truncation and strong chunks can no longer be cut arbitrarily.
  IF and_part IS NOT NULL THEN
    BEGIN
      ts_precise := to_tsquery('simple', and_part);
    EXCEPTION WHEN OTHERS THEN
      ts_precise := NULL;
    END;
  END IF;

  IF ts_precise IS NOT NULL THEN
    SELECT count(*) INTO precise_cnt FROM (
      SELECT 1
      FROM public.legal_document_chunks c2
      WHERE to_tsvector('simple', c2.content) @@ ts_precise
      LIMIT precise_rank_budget + 1
    ) s;
    IF precise_cnt > precise_rank_budget THEN
      ts_precise := NULL;
    END IF;
  END IF;
  ...
  precise_cands AS (
    SELECT
      ...
      ts_rank(to_tsvector('simple', c2.content), ts) AS content_rank
    FROM public.legal_document_chunks c2
    WHERE ts_precise IS NOT NULL
      AND to_tsvector('simple', c2.content) @@ ts_precise
    ORDER BY ts_rank(to_tsvector('simple', c2.content), ts) DESC, c2.id ASC
    LIMIT 2000
  ),
  broad_cands AS (
    SELECT
  ...
  chunk_cands AS (
    SELECT cid, did, max(content_rank) AS content_rank
    FROM (
      SELECT cid, did, content_rank FROM precise_cands
      UNION ALL
      SELECT cid, did, content_rank FROM broad_cands
    ) u
    GROUP BY cid, did
  ),

// Stability test for v7.9 — Milestone A.5 (source-pack classification fix).
...
const RUN_ID = `stability-v79-${randomUUID()}`;
const OUT_FILE = `${OUT_DIR}/stability-v7.9.json`;
const LOG_FILE = `${OUT_DIR}/stability-v7.9.log`;
...
function summarize(state) {
  const lines = [];
  lines.push(`# Stability v7.9 (Milestone A.5 — source-pack classification fix) — ${state.run_id}`);
  lines.push(``);
...
async function main() {
  ...
  writeFileSync(`${OUT_DIR}/stability-v7.9.summary.md`, summary);
  log(`=== DONE ===\n${summary}`);
}

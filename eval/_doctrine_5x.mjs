...
async function runOne(i) {
  ...
  for (let k = 0; k < 90; k++) {
    ...
    const { data: rows } = await admin.from("qa_logs")
      .select("id,answer,footnotes,total_footnotes,metadata,created_at,question")
      .eq("user_id", UID)
  ...
  const core = md.core || {};
  const dc = core.doctrine_classifier;
  const sr = core.source_requirements;
  const cq = core.citation_quality || md.citation_quality;
  const stageRuns = core.stage_runs || md.stage_runs || [];
  const dcRun = stageRuns.find((s) => s.stage === "doctrine_classifier");
  const out = {
    run: i,
    qa_log_id: row.id,
    ...
    footnotes_count: row.total_footnotes ?? (row.footnotes || []).length,

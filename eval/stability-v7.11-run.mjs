// Stability test for v7.11 — Milestone B (Stage E.5 Perplexity completion).
// Captures Stage E.5 telemetry from metadata.retrieval_funnel.perplexity_completion:
//   - triggered, status, candidates_returned, candidates_kept
//   - core_before vs core_after
//   - drop reasons per candidate
//   - final anchored count in body
...
const RUN_ID = `stability-v711-${randomUUID()}`;
const OUT_FILE = `${OUT_DIR}/stability-v7.11.json`;
const LOG_FILE = `${OUT_DIR}/stability-v7.11.log`;
...
function summarize(state) {
  const lines = [];
  lines.push(`# Stability v7.11 (Milestone B — Stage E.5 Perplexity Completion) — ${state.run_id}`);
  lines.push(``);
  lines.push(`| Q | r | path | E5? | ret | kept | core_b→a | anch | wall |`);
  lines.push(`|---|---|---|---|---|---|---|---|---|`);
  ...
  for (const q of state.questions) {
    q.runs.forEach((r, i) => {
      ...
      lines.push(`| Q${q.id} | r${i+1} | ${r.drafting_path ?? "ERR"} | ${e5} | ${r.e5_candidates_returned ?? "—"} | ${r.e5_candidates_kept ?? "—"} | ${r.e5_core_before ?? "—"}→${r.e5_core_after ?? "—"} | ${r.anchored_count} | ${(r.wall_ms/1000).toFixed(1)}s |`);
    });
  }
  lines.push(``);
  if (tot.all > 0) {
    ...
    lines.push(`- Total candidates kept (passed all 3 guards): ${tot.e5_kept}`);
...
async function main() {
  log(`=== Stability v7.11 run ${RUN_ID} ===`);
  log("Authenticating…");
  ...
  writeFileSync(`${OUT_DIR}/stability-v7.11.summary.md`, summary);
  log(`=== DONE ===\n${summary}`);
}

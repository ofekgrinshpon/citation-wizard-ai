// Stability test for v7.7 Fast-mode output shaping.
...
const RUN_ID = `stability-v77-${randomUUID()}`;
const OUT_FILE = `${OUT_DIR}/stability-v7.7.json`;
const LOG_FILE = `${OUT_DIR}/stability-v7.7.log`;
...
function summarize(state) {
  const lines = [];
  lines.push(`# Stability v7.7 (Fast-mode shaping) — ${state.run_id}`);
  lines.push(``);
...
async function main() {
  ...
  writeFileSync(`${OUT_DIR}/stability-v7.7.summary.md`, summary);
  log(`=== DONE ===\n${summary}`);
}

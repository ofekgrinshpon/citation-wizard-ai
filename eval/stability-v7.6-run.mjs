// Stability test for v7.6 Fast-mode output shaping.
...
const RUN_ID = `stability-v76-${randomUUID()}`;
const OUT_FILE = `${OUT_DIR}/stability-v7.6.json`;
const LOG_FILE = `${OUT_DIR}/stability-v7.6.log`;
...
function summarize(state) {
  const lines = [];
  lines.push(`# Stability v7.6 (Fast-mode shaping) — ${state.run_id}`);
  lines.push(``);
...
async function main() {
  ...
  writeFileSync(`${OUT_DIR}/stability-v7.6.summary.md`, summary);
  log(`=== DONE ===\n${summary}`);
}

// Step 4c: sequential Q1/Q3/Q5 probe with robust polling.
// Polls by eval_run_id first; falls back to (question, user_id, created_at>=submission).
...
const ONLY = process.env.ONLY ? process.env.ONLY.split(",") : ["Q1", "Q3", "Q5"];
const QUERIES = [
...
for (const { id, q } of QUERIES) {
  const evalRunId = `step4c-${id}-${randomUUID().slice(0, 8)}`;
  ...
  results.push({
    ...
    per_anchor: perAnchor.map((p) => ({
      ...
      exact_lookup_detail: p.exact_lookup_detail ? {
        matched_by: p.exact_lookup_detail.matched_by,
        confidence: p.exact_lookup_detail.confidence ?? p.local_confidence,
        lookup_queries_attempted: p.exact_lookup_detail.lookup_queries_attempted,
      } : null,
  ...
  console.log(`${id} ok matched_by=${found.matched_by} v3=${md.v3_path} fn=${(row.footnotes || []).length}`);
}

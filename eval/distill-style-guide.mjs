...
// ─── 4. Synthesize ──────────────────────────────────────────────────────────
const synthSystem = `אתה עורך סגנון בכיר. תקבל ${observations.length} תצפיות מובנות שזוקקו ממאמרים אקדמיים משפטיים בעברית. עליך לכתוב מסמך הנחיות סגנון יחיד, מקיף, מצרכני (prescriptive), בעברית, באורך עד 12,000 תווים (≈3,000 טוקנים). אסור לך לחרוג מהתקרה. השתמש בכותרות **מודגשות** (לא markdown #). שמונה סעיפים בדיוק:

...
- [Manual Outline Edit](mem://features/academic-writing-mode/manual-outline-edit) — `OutlineChapterEditor` lets user rename/reorder/add/delete body chapters before approval; `approveOutline(editedBodyTitles?)` accepts edited list; special chapters always force-injected
- [Topic Reality Check](mem://features/academic-writing-mode/topic-reality-check) — `suggest_topics` runs planner (gpt-5-mini) → hybrid local retrieval (text + embedding/match_legal_chunks) → Perplexity sonar fallback when DB hits<MIN_HITS; injects verified `[מאגר]/[חיצוני]` source pool into LLM prompt; frontend badges + sources accordion; flags `TOPIC_REALITY_CHECK_ENABLED`/`TOPIC_REALITY_PPLX_ENABLED`/`TOPIC_REALITY_MIN_HITS`
- [Research V2 Pipeline](mem://architecture/legal-qa-v2-pipeline) — Simplified 5-stage pipeline (ResearchPlan → per-claim retrieval → verification → ledger → drafter) behind RESEARCH_V2 flag; new modules: researchPlan.ts / claimRetrieval.ts / ledger.ts; full plan in .lovable/plan.md
- [Database Name URL Mapping](mem://logic/citation-rules/database-name-url-mapping) — Maps Perplexity source URL host → canonical Rule 19.1 database name (lite.takdin → תקדין, supremedecisions.court.gov.il → אר״ש, nevo → נבו, psakdin → פסקדין); allowed set: נבו/פדאור/דינים/תקדין/אר״ש/פסקדין; mirrored in citation-chat, citationValidation.ts, and both citationEngine.ts copies
- [Deep Research Resume](mem://logic/legal-qa/deep-research-resume) — Deep research uses Pass E async (no SSE); client mints runId + localStorage marker BEFORE fetch; resume effect on remount polls legal-qa-status; backend dedupes by runId so duplicate clicks reattach instead of charging twice
- [Batch Verifier](mem://logic/legal-qa/batch-verifier) — One LLM call per claim (cap 24) with legacy chunk-of-8 fallback; rich telemetry incl. naive_per_candidate; validated 2026-05-22
